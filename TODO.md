...
- **Auto-suspend on physical switch open/close** — x4N70pHyLL's request from #33: when the user fully opens the cover via a physical switch, suspend automatic control until they fully close it again. Open design questions before scoping a PR: (1) generalize to all cover types, or stay venetian-only? (2) interaction with `manual_override` — separate state, or sub-mode of override? (3) per-cover setting vs global behavior? (4) fire on any "fully open" event or only when the change is user-initiated (not from us)? File a new issue once those are decided. v2.20.0 release notes call this out as planned.
- **Per-axis override config** for venetian — today `force_override_position`, `custom_position_N`, climate position, sunset_position are scalar (set position; tilt is engine-derived). Adding an optional companion (`force_override_tilt`, `custom_position_N_tilt`, etc.) lets users force a specific slat angle independent of position. Larger config surface; defer until users ask.
- **Cloud-suppression staged tilt** (Issue #175) — now unblocked by dual-axis venetian. On clouds, instead of opening the cover fully, set tilt=100% (open slats) and hold position; after the configured offset, open the cover. Pick up #175 separately.
- **Glare zones for venetian** — currently BLIND-only. Position calculation is the same vertical math, so adding glare-zone support to venetian is mechanical. Defer until requested.
- **Guided merge flow in config_flow** — for users with paired cover_blind + cover_tilt instances against the same entity, detect the pair and offer a one-click migration to a single cover_venetian instance. v2.20.0 ships doc-only migration; revisit if the wiki page proves insufficient.
- **Venetian tilt axis position limits** (Issue #33 follow-up) — `max_position`/`min_position` constrain the carriage axis only. After one beta cycle with the doc clarification, evaluate whether to add `CONF_VENETIAN_TILT_MAX_POSITION`/`CONF_VENETIAN_TILT_MIN_POSITION`. Would require: range constants in `const.py`, entries in `OPTION_RANGES`, schema in `GEOMETRY_VENETIAN_SCHEMA`, application in `engine/covers/venetian.py:_compute_tilt`, and a regression test that slat angle is clamped before being threaded through `tilt_for_position`.
